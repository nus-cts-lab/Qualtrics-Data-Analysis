...
print("\n" + "="*80)
print("ANALYZING ORIGINAL WSAP (Columns DO-DT)")
print("="*80)
...
print("\n" + "="*80)
print("ANALYZING NEW WSAP (Columns BW-BZ)")
print("="*80)
...
# Display results
print("\n=== COMBINED WSAP ANALYSIS RESULTS ===\n")
print(combined_df.to_string(index=False))

# EXPORT RESULTS AND DDM DATA
...
if original_ddm_data:
    ...
    print(f"✓ Original WSAP DDM data saved to: {ddm_file} ({len(original_ddm_combined)} trials)")
...
if new_ddm_data:
    ...
    print(f"✓ New WSAP DDM data saved to: {ddm_file} ({len(new_ddm_combined)} trials)")
...
if len(orig_rss_numeric) > 0:
    print(f"Response Selection Score: M={orig_rss_numeric.mean():.3f}, SD={orig_rss_numeric.std():.3f} (n={len(orig_rss_numeric)})")
else:
    print("Response Selection Score: No valid data")

if len(orig_rt_numeric) > 0:
    print(f"RT Bias Index: M={orig_rt_numeric.mean():.3f}, SD={orig_rt_numeric.std():.3f} (n={len(orig_rt_numeric)})")
else:
    print("RT Bias Index: No valid data")
...
if len(new_rss_numeric) > 0:
    print(f"Response Selection Score: M={new_rss_numeric.mean():.3f}, SD={new_rss_numeric.std():.3f} (n={len(new_rss_numeric)})")
else:
    print("Response Selection Score: No valid data")

if len(new_rt_numeric) > 0:
    print(f"RT Bias Index: M={new_rt_numeric.mean():.3f}, SD={new_rt_numeric.std():.3f} (n={len(new_rt_numeric)})")
else:
    print("RT Bias Index: No valid data")

print("\n" + "="*80)
print("NOTE: DDM parameters (drift rate, starting point, threshold) require")
print("specialized modeling software like HDDM or PyDDM.")
print("The trial-level data has been prepared for DDM analysis.")
print("="*80)